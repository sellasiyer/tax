...
public class TaxClient extends WebServiceGatewaySupport{
    ...
    private JAXBElement<QuotationRequest> getTaxQuotationRequest() {
        ...
        quotationRequest.setDocumentNumber("1");
        ...
        for(int i =1; i<4 ;i++) {
            ...
            lineItem.setLineItemId(Integer.toString(i));
            lineItem.setExtendedPrice(BigDecimal.valueOf(Long.valueOf("100")));
            lineItem.setCustomer(customer);
            lineItem.setQuantity(quantity);

            LineItem.Product product = objectFactory.createLineItemProduct();
            product.setValue(Integer.toString(i));
            ...
            flexibleFields.setOriginalSellingPrice(lineItem.getExtendedPrice().toString());
